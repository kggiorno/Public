...
def defineCities():
    urlList = []
    '''citiesList = ['atlanta','tallahassee', 'chattanooga', 'myrtlebeach', 
    'houston', 'easttexas', 'santabarbara', 'norfolk', 'sandiego', 'newjersey', 
    'fresno', 'annapolis', 'montana', 'waco', 'neworleans', 'lasvegas', 
    'spokane', 'phoenix', 'tampa', 'milwaukee', 'dallas', 'newhaven', 'newyork', 
    'daytona', 'sd', 'rochester', 'detroit', 'richmond', 'philadelphia', 
    'florence', 'hartford', 'columbia', 'sfbay', 'denver', 'raleigh', 
    'saltlakecity', 'charlottesville', 'amarillo', 'iowacity', 'greenville', 
    'collegestation', 'boone', 'albuquerque', 'omaha', 'tricities', 
    'lexington','charlestonwv', 'savannah', 'cleveland', 'bend', 'madison', 
    'charleston', 'asheville', 'albany', 'harrisburg', 'staugustine', 
    'columbus', 'sacramento', 'lafayette', 'pennstate', 'jerseyshore', 
    'fortcollins', 'tucson', 'washingtondc', 'baltimore', 'indianapolis', 
    'austin', 'oklahomacity', 'birmingham', 'gainesville', 'delaware', 
    'jackson', 'longisland','provo',  'gulfport', 'seattle', 'minneapolis', 
    'athensga', 'louisville', 'portland', 'orangecounty', 'miami', 'boston', 
    'jacksonville','orlando','knoxville', 'boulder','chicago', 'vermont', 
    'reno','cincinnati','nashville', 'charlotte', 'wyoming','maine', 
    'florencesc', 'sanantonio', 'losangeles', 'augusta', 'honolulu', 'fortlauderdale']
    '''
    citiesList = ['austin','charleston','seattle']
    telecommuteTrue = False
    telecommuteCheck = input('Would you like to filter for only telecommuting '
        ' jobs? : Y = Yes, any other response = No')
    if telecommuteCheck.lower() == 'y':
        for city in citiesList:
            cityURL = 'http://%s.craigslist.org/search/jjj?is_telecommuting=1' % city
            urlList.append(cityURL)
    else:
        for city in citiesList:
            cityURL = 'http://%s.craigslist.org/search/jjj' % city
            urlList.append(cityURL)
    return urlList
